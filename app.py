...
LANG = {
    "IT": {
        ...
        "stat_err": "❌ Errore", "stat_err_time": "❌ Errore (Tempo < 5)", "stat_err_invalid": "❌ Errore (Tempo non valido)", "stat_ok": "✅ OK"
    },
    "DE": {
        "title": "🤖 Global Data Automation Agent",
        "subtitle": "Data transfer with **Date Correction**, **Dynamic ZIP Search**, **Filters**, and **Safe Saving**.",
        ...
        "stat_err": "❌ Fehler", "stat_err_time": "❌ Fehler (Zeit < 5)", "stat_err_invalid": "❌ Fehler (Ungültige Zeit)", "stat_ok": "✅ OK"
    }
}
...
# --- 0. CALCOLO DATA DI RIFERIMENTO E CALENDARIO SETTIMANA ---
...
if zip_file and master_file and percorso_selezionato:
    ...
    with zipfile.ZipFile(zip_file, 'r') as z:
        ...
        df_main = pd.DataFrame(rows, columns=colonne_base)

        def check_row(row):
            if pd.isna(row[L["col_tech"]]) or pd.isna(row[L["col_code"]]) or row[L["col_tech"]] == "❓":
                return L["stat_err"]
            ...
            if str(row[L["col_tech"]]).upper() in valid_tech_ids and str(row[L["col_code"]]).upper() in valid_error_codes:
                return L["stat_ok"]
            return L["stat_err"]

        if not df_main.empty:
            df_main[L["col_status"]] = df_main.apply(check_row, axis=1)
        else:
            df_main[L["col_status"]] = pd.Series(dtype=str)
        ...
        if not df_main.empty and not df_edited_filtered.empty:
            df_main.update(df_edited_filtered)
